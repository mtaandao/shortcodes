...
if ( !class_exists( 'Sunrise4' ) ) {
	...
	class Sunrise4 {
		...
		function __construct( $args = array() ) {
			...
			if ( !$args['file'] ) mn_die( 'Sunrise: please specify plugin __FILE__' );
		...
		public function add_menu( $args ) {
			...
			$args = mn_parse_args( $args, array(
					...
					'icon_url'    => admin_url( 'images/mn-logo.png' ),
		...
		public function render() {
			...
			echo '<input type="hidden" name="sunrise_nonce" value="' . mn_create_nonce( 'sunrise_nonce' ) . '" />';
		...
		public function assets() {
			// Register styles
			mn_register_style(  'sunrise', plugins_url( $this->config['css'], $this->config['file'] ) . '/sunrise.css', false, $this->config['version'], 'all' );
			// Register scripts
			mn_register_script( 'form',    plugins_url( $this->config['js'],  $this->config['file'] ) . '/form.js', array( 'jquery' ), $this->config['version'], true );
			mn_register_script( 'sunrise', plugins_url( $this->config['js'],  $this->config['file'] ) . '/sunrise.js', array( 'form' ), $this->config['version'], true );
			// Add some l10n to JS
			mn_localize_script( 'sunrise', 'sunrise', array(
		...
		public function enqueue() {
			...
			// Enqueue styles
			foreach ( array( 'farbtastic', 'sunrise' ) as $style ) mn_enqueue_style( $style );
			// Enqueue scripts
			foreach ( array( 'jquery', 'farbtastic', 'form', 'sunrise' ) as $script ) mn_enqueue_script( $script );
		...
		public function submit() {
			...
			if ( !$this->is_sunrise() ) return;
			// Check nonce
			if ( !isset( $_REQUEST['sunrise_nonce'] ) || !mn_verify_nonce( $_REQUEST['sunrise_nonce'], 'sunrise_nonce' ) ) return;
			// Submit hooks
			...
			case 'save': // Save options
				...
				mn_redirect( $this->get_page_url() . '&message=1' );
				exit;
			case 'reset': // Reset options
				...
				mn_redirect( $this->get_page_url() . '&message=2' );
				exit;
			}
		}
...
if ( !class_exists( 'Sunrise4_Views' ) ) {
	...
	class Sunrise4_Views {
		...
		public static function closetab( $field, $config ) {
			$field = mn_parse_args( $field, array( 'actions' => true ) );
			$return = array();
			$return[] = '</table>';
			if ( $field['actions'] ) $return[] = '<div class="sunrise-actions-bar"><input type="submit" value="' . __( 'Save changes', $config['textdomain'] ) . '" class="sunrise-submit button-primary" /><span class="sunrise-spin"><img src="' . admin_url( 'images/mnspin_light.gif' ) . '" alt="" /> ' . __( 'Saving', $config['textdomain'] ) . '&hellip;</span><span class="sunrise-success-tip"><img src="' . admin_url( 'images/yes.png' ) . '" alt="" /> ' . __( 'Saved', $config['textdomain'] ) . '</span><a href="' . $_SERVER["REQUEST_URI"] . '&amp;sunrise_action=reset&amp;sunrise_nonce=' . mn_create_nonce( 'sunrise_nonce' ) . '" class="sunrise-reset button alignright" title="' . esc_attr( __( 'This action will delete all your settings. Are you sure? This action cannot be undone!', $config['textdomain'] ) ) . '">' . __( 'Restore default settings', $config['textdomain'] ) . '</a></div>';
			$return[] = '</div>';
		...
		public static function media( $field, $config ) {
			...
			if ( function_exists( 'mn_enqueue_media' ) ) mn_enqueue_media();
