...
function su_image_resize( $url, $width = NULL, $height = NULL, $crop = true, $retina = false ) {
	...
	//######################################################################
	if ( isset( $mn_version ) && version_compare( $mn_version, '3.5' ) >= 0 ) {
		global $mndb;
		if ( empty( $url ) )
			return new MN_Error( 'no_image_url', 'No image URL has been entered.', $url );
		...
		if ( is_multisite() ) {
			...
			$file_path = str_replace( $blog_details->path . 'files/', '/main/blogs.dir/' . $blog_id . '/files/', $file_path );
		}
		...
		$dir = $info['dirname'];
		$ext = $info['extension'];
		$name = mn_basename( $file_path, ".$ext" );
		// Suffix applied to filename
		$suffix = "{$dest_width}x{$dest_height}";
		// Get the destination file name
		$dest_file_name = "{$dir}/{$name}-{$suffix}.{$ext}";
		if ( !file_exists( $dest_file_name ) ) {
			$query = $mndb->prepare( "SELECT * FROM $mndb->posts WHERE guid='%s'", $url );
			$get_attachment = $mndb->get_results( $query );
			if ( !$get_attachment )
				return array( 'url' => $url, 'width' => $width, 'height' => $height );
			// Load Wordpress Image Editor
			$editor = mn_get_image_editor( $file_path );
			if ( is_mn_error( $editor ) )
				return array( 'url' => $url, 'width' => $width, 'height' => $height );
			...
			$metadata = mn_get_attachment_metadata( $get_attachment[0]->ID );
			if ( isset( $metadata['image_meta'] ) ) {
				$metadata['image_meta']['resized_images'][] = $resized_width . 'x' . $resized_height;
				mn_update_attachment_metadata( $get_attachment[0]->ID, $metadata );
			}
	...
	//######################################################################
		...
		if ( !file_exists( $dest_file_name ) ) {
			...
			$image = mn_load_image( $file_path );
			if ( !is_resource( $image ) )
				return new MN_Error( 'error_loading_image_as_resource', $image, $file_path );

			// Get the current image dimensions and type
			$size = @getimagesize( $file_path );
			if ( !$size )
				return new MN_Error( 'file_path_getimagesize_failed', 'Failed to get $file_path information using getimagesize.' );
			list( $orig_width, $orig_height, $orig_type ) = $size;

			// Create new image
			$new_image = mn_imagecreatetruecolor( $dest_width, $dest_height );
			...
			if ( IMAGETYPE_GIF == $orig_type ) {
				if ( !imagegif( $new_image, $dest_file_name ) )
					return new MN_Error( 'resize_path_invalid', 'Resize path invalid (GIF)' );
			}
			elseif ( IMAGETYPE_PNG == $orig_type ) {
				if ( !imagepng( $new_image, $dest_file_name ) )
					return new MN_Error( 'resize_path_invalid', 'Resize path invalid (PNG).' );
			}
			else {
				...
				if ( !imagejpeg( $new_image, $dest_file_name, apply_filters( 'resize_jpeg_quality', 90 ) ) )
					return new MN_Error( 'resize_path_invalid', 'Resize path invalid (JPG).' );
			}
			...
			if ( !$new_size )
				return new MN_Error( 'resize_path_getimagesize_failed', 'Failed to get $dest_file_name (resized image) info via @getimagesize', $dest_file_name );
...
function su_delete_resized_images( $post_id ) {
	...
	$metadata = mn_get_attachment_metadata( $post_id );
	if ( !$metadata )
		return;
	...
	$mn_upload_dir = mn_upload_dir();
	$upload_dir = $mn_upload_dir['basedir'];
...
class Su_Tools {
	function __construct() {
		add_action( 'mn_ajax_su_example_preview', array( __CLASS__, 'example' ) );
	...
	public static function get_slides( $args ) {
		...
		if ( is_array( $query->posts ) ) foreach ( $query->posts as $post ) {
				...
				$slide = array(
					'image' => mn_get_attachment_url( $thumb ),
					'link'  => '',
	...
	public static function example() {
		...
		self::access();
		// Check nonce
		if ( !isset( $_REQUEST['nonce'] ) || !mn_verify_nonce( $_REQUEST['nonce'], 'su_examples_nonce' ) ) return;
		// Check incoming data
		...
		// Cache not found
			...
			foreach ( $chunks as $chunk ) {
				...
				echo mnautop( do_shortcode( $chunk ) );
	...
	public static function access() {
		if ( !self::access_check() ) mn_die( __( 'Access denied', 'shortcodes-ultimate' ) );
	}
