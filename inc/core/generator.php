...
class Su_Generator {
	...
	function __construct() {
		...
		add_action( 'mn_ajax_su_generator_settings',       array( __CLASS__, 'settings' ) );
		add_action( 'mn_ajax_su_generator_preview',        array( __CLASS__, 'preview' ) );
		add_action( 'su/generator/actions',                array( __CLASS__, 'presets' ) );

		add_action( 'mn_ajax_su_generator_get_icons',      array( __CLASS__, 'ajax_get_icons' ) );
		add_action( 'mn_ajax_su_generator_get_terms',      array( __CLASS__, 'ajax_get_terms' ) );
		add_action( 'mn_ajax_su_generator_get_taxonomies', array( __CLASS__, 'ajax_get_taxonomies' ) );
		add_action( 'mn_ajax_su_generator_add_preset',     array( __CLASS__, 'ajax_add_preset' ) );
		add_action( 'mn_ajax_su_generator_remove_preset',  array( __CLASS__, 'ajax_remove_preset' ) );
		add_action( 'mn_ajax_su_generator_get_preset',     array( __CLASS__, 'ajax_get_preset' ) );
	}
	...
	public static function popup() {
		...
		// Cache not found
			...
			$tools = apply_filters( 'su/generator/tools', array(
					...
					'<a href="http://mtaandao.org/support/plugin/shortcodes-ultimate/" target="_blank" title="' . __( 'Support forums', 'shortcodes-ultimate' ) . '">' . __( 'Support forums', 'shortcodes-ultimate' ) . '</a>'
				) );
...
?>
	...
	public static function settings() {
		self::access();
		// Param check
		if ( empty( $_REQUEST['shortcode'] ) ) mn_die( __( 'Shortcode not specified', 'shortcodes-ultimate' ) );
		// Get cache
		...
		// Cache not found
			...
			if ( isset( $shortcode['note'] ) || isset( $shortcode['example'] ) ) {
				...
				if ( isset( $shortcode['example'] ) ) $return .= mnautop( '<a href="' . admin_url( 'admin.php?page=shortcodes-ultimate-examples&example=' . $shortcode['example'] ) . '" target="_blank">' . __( 'Examples of use', 'shortcodes-ultimate' ) . ' &rarr;</a>' );
				$return .= '</div></div>';
			}
			// Shortcode has atts
